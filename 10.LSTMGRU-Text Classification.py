...
class MultiLayerLSTM:
	...
	def __init__(self, teacher_forcing=False):
		...
		self.timestamp = time()
		self.log_dir = "./tf_logs/lstm_classification/" + str(self.timestamp) +"_model/"
...
#		 return tf.keras.losses.binary_crossentropy(y_true, y_pred)
	...
	def save_model(self):
		model.save_weights('lstm_classification_' + str(self.timestamp), save_format='tf')
		return
	...
	def setup_model(self, hparams, single_layer=True):
		self.generate_model(params, single_layer)
		self.compile_model(self.loss_function, hparams[self.optimizer])
		return self.model
...
#		 return tf.keras.losses.binary_crossentropy(y_true, y_pred)
	...
	def save_model(self):
		model.save_weights('gru_classification_' + str(self.timestamp), save_format='tf')
		return
